...
def create_new_bill(data: BillCreate, db: Session = Depends(get_db)):
    try:
...
def update_existing_bill(bill_id: int, data: BillUpdate, db: Session = Depends(get_db)):
...
def delete_existing_bill(bill_id: int, db: Session = Depends(get_db)):
